...
function getParagraphNode(lines: string[], start: number): {pnode: ParagraphNode; endindex: number} {
...
function parseBulletedLine(line: string): MarkerByLanguage | null {
...
function getBulletBlock(lines: string[], start: number): {bulletblock: BulletBlock, endIndex: number} {
    ...
    while (i < lines.length && isBulletedLine(lines[i])) {
        const bulletItemNode: BulletItemNode = {
            kind: "BulletItem",
            content: [[lines[i]]],
            markerByLanguage: parseBulletedLine(lines[i]) ?? undefined
        };
...
function getNumberedBlock(lines: string[], start: number): {numberedblock: NumberedBlock, endIndex: number} {
